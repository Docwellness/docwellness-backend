...
COMPONENTS RULE (this is what the dietician and patient actually see and adjust - get it right): "components" describes ONE serving of the dish the way a dietician would actually prescribe it and a patient would actually count it, NOT a generic weight. Break the dish into its real, separately-servable/countable parts and give each its own natural unit - never force something countable or vessel-served into grams just because grams is available:
  ...
  - List every component a patient would recognize as a distinct part of the plate - NEVER more than 3, and 1 is correct far more often than not. A simple single-part dish (which is most dishes, including every sabji/dal/curry/bhurji/chaat/salad/soup/drink) gets exactly ONE components entry, repeating the dish name as its label with one total quantity - do NOT break a single dish into its individual ingredients as separate components (onion, tomato, spices, oil, garnish are ingredients IN the dish, not separate parts of the plate). Only use 2-3 entries when the dish is ACTUALLY served as genuinely separate, independently-servable dishes on the same plate (e.g. Idli + Sambar + Chutney are 3 separate dishes physically served apart, each countable/scoopable on its own).
...
- A dish that's genuinely a single scoopable/pourable mass with no natural count uses "g" or "ml": "Oats Porridge" -> [{"label":"Oats Porridge","quantity":250,"unit":"g"}], "Buttermilk" -> [{"label":"Buttermilk","quantity":200,"unit":"ml"}].
- "nos" = a generic countable unit for items with no more specific unit of their own (idli, dumpling, cutlet); "bowl"/"cup"/"piece"/"slice"/"egg"/"tbsp"/"tsp" when they're the natural way that specific item is served/counted; "g"/"ml" only for genuinely unitless masses/liquids.
- List every component a patient would recognize as a distinct part of the plate - NEVER more than 3, and 1 is correct far more often than not. A simple single-part dish (which is most dishes, including every sabji/dal/curry/bhurji/chaat/salad/soup/drink) gets exactly ONE components entry, repeating the dish name as its label with one total quantity - do NOT break a single dish into its individual ingredients as separate components. WRONG (never do this): "Masoor Dal" -> [{"label":"Masoor Dal"},{"label":"Onion"},{"label":"Tomato"},{"label":"Ginger"},{"label":"Garlic"},{"label":"Turmeric Powder"},{"label":"Salt"},{"label":"Oil"},{"label":"Coriander Leaves"}] - onion/tomato/spices/oil/garnish are ingredients IN the dal, not separate parts of the plate. RIGHT: "Masoor Dal" -> [{"label":"Masoor Dal","quantity":250,"unit":"g"}], one entry, full stop. Only use 2-3 entries when the dish is ACTUALLY served as genuinely separate, independently-servable dishes on the same plate (e.g. "Idli with Sambar and Chutney" is 3 separate dishes physically served apart - Idli, Sambar, Chutney - each countable/scoopable on its own).
